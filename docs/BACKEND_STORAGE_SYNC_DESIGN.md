...
- Python rednote解析：`backend/app/xhs/parser.py`。
...
- rednote Web 端已不稳定返回 `originVideoKey`，当前方案不再采集或输出该字段；视频能力保存平台返回的结构化 `streams[]` 候选流和 `bizId`。
...
- 前端写操作携带 `baseRevision`，后端对收藏、编辑、删除、清空、导入、重新抓取做 revision 校验。
- 旧页面提交时返回 `409 CONFLICT` 和 `currentRevision`，前端自动拉取最新数据并提示用户重新确认。
- 编辑弹窗遇到 `409` 时保留草稿，用户需要基于最新数据再次保存。
- 上传前校验云端 revision 是否仍等于本地 `sync-base.json` 的 revision，避免多设备或其他实例静默覆盖云端新数据。
- 本地和云端都只新增不同收藏时，自动合并为一个新 revision 后上传。
- 同一条收藏被两端修改，或涉及删除、清空等无法证明安全的变更时，进入 `remote_conflict`，前端展示“拉取云端”和“覆盖云端”。
- 拉取云端会先备份本地当前 JSON；覆盖云端会先备份远端 JSON 和本地 JSON。
...
- 旧 `baseRevision` 发起收藏、编辑、删除、清空、导入、重新抓取时返回 `409 CONFLICT`。
- 收藏和重新抓取在旧 `baseRevision` 下先做本地 revision 校验，不先访问rednote解析。
- `POST /api/sync/push` 上传前拉取云端并比较 `sync-base.json` revision。
- 云端和本地都是纯新增时自动合并并上传，最终 JSON 包含两边新增收藏。
- 无法自动合并时保留本地 dirty 状态，`GET /api/sync/status` 返回 `status=remote_conflict`。
- `POST /api/sync/pull` 用云端覆盖本地前会写入 `data/local-backups/`。
- `POST /api/sync/push` 携带 `force=true` 覆盖云端前会备份远端和本地数据。

前端：

- 所有写请求携带当前 `baseRevision`。
- 收到 `409` 后重新拉取收藏和同步状态，并提示“数据已在其他页面更新”。
- 编辑保存遇到 `409` 时保留用户输入。
- `remote_conflict` 状态下隐藏普通“保存并上传”，展示“拉取云端”和“覆盖云端”。
...
| rednote页面结构变化 | 解析失败 | 错误分层；保留重新抓取；记录失败原因 |
...
- [x] `AC-P0-015` `/api/sample` 可以返回多图rednote示例。
- [x] `AC-P0-016` `/api/collect` 可以实际解析rednote示例并写入收藏。
