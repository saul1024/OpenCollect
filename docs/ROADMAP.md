...
| P2 | 解析稳定性、去重、重新抓取 | Next | 让收藏失败原因清楚，重复收藏可识别，旧收藏可刷新元数据 | 重复链接提示已存在；失败原因分层；支持重新抓取并更新标题/媒体/统计 |
...
- [ ] `OC-P2-008` 评估并实现结构化视频流字段，例如 `streams[]`、`media.video.bizId`、`mediaV2.video.biz_id` 或 stream path。
